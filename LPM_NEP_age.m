...
% This code is hardcoded to use day 1 data from NEP LES simulations
%--------------------------------------------------------------------------
% This version calculates the Twomey forcing as a function of particle age
%--------------------------------------------------------------------------
% version to calculate repeated diurnal cycles 
%--------------------------------------------------------------------------
...
C_0_prec = 0.15; % C_0 for precipitating cases
C_0_noprec = 0.5; % C_0 for non-precipitating cases

% Particles decay to represent wet/dry deposition processes on an e-folding timescale
% set by tau_decay (default = 2 days)
tau_decay = 4.0; % days
...
num_ships = 2; % number of ships
part_num = 500; % number of particles injected at each time step

% Domain size
x_range = 200; % km
y_range = 200; % km

dt = 600; % Particle model time step in seconds
injection_rate = 10^16; % particles per second
rho_mg = 1.15*10^6; % g/m^3
...
% Plotting parameters
save_interval = 1200; % in seconds [this matches the 2-D LES data for Twomey forcing calc.]
day_s = 86400; % seconds in a day
num_days = 10; % number of diurnal cycles
...
injection_offsets = [0]; % Options for hours relative to dawn: -4, 0, +4
num_cases = 4; % Number of particle age categories: young, middle, old, all
...
rad_forcing_full = zeros(x_blocks, y_blocks, save_size, num_cases, num_les_cases);
...
for les_idx = 1:num_les_cases
    ...
    SOLIN_day1 = SOLIN(:,:,1:73);
    SOLIN_diurnal = SOLIN(:,:,2:73);
    SWNT_day1 = SWNT(:,:,1:73);
    SWNT_diurnal = SWNT(:,:,2:73);
    ...
    injection_periods = cell(1, num_dawns); % Single injection case, multiple dawns
    for d = 1:num_dawns
        start_time = new_time(dawn(d));
        end_time = start_time + injection_duration;
        if end_time > new_time(end)
            end_time = new_time(end);
        end
        injection_periods{d} = [start_time, end_time];
    end

    % Initialize arrays (single run for all particles)
    part_pos_x = [];
    part_pos_y = [];
    part_vel_u = [];
    part_vel_v = [];
    ...
    %0 m/s ship velocity
    %ship_motion_y_options = [0.2, 0, 0.2];
    %ship_motion_x_options = [0.2, 0, 0.2];
    ...
    for n = 1:length(new_time)
        ...
        % Check if current time is within an injection period
        is_injecting = false;
        for d = 1:num_dawns
            period = injection_periods{d};
            if (current_time >= period(1) && current_time <= period(2))
                is_injecting = true;
                %fprintf('Injecting at: %.4f hours (period %d)\n', current_time / 3600, d);
                break;
            end
        end
        ...
        if mod(current_time, save_interval) == 0
            ...
            % Save 2D particle concentration for all age categories (same concentration for now)
    ...
    for case_idx = 1:num_cases
        for t = dawn_Twomey(num_days):dusk_Twomey(num_days+1)
            ...
            % Compute delta_ac for each age category
            Nd_background = NAc_bl_repeated(t); %account for varying background aerosol
            phi = 1.0; % Atmospheric correction factor
            delta_ac = zeros(200, 200, num_cases);

            % Categorize particles by age at the current time
            current_time_hours = time_les(t) / 3600.0;
            particle_ages = current_time_hours - part_release_time; % Age in hours
            young_indices = find(particle_ages >= 0 & particle_ages <= 24); % 0-1 days
            middle_indices = find(particle_ages > 24 & particle_ages <= 72); % 1-3 days
            old_indices = find(particle_ages > 72); % 3+ days
            all_indices = 1:length(part_release_time); % All particles

            % Compute particle concentrations for each age category
            age_categories = {young_indices, middle_indices, old_indices, all_indices};
            %age_categories = {all_indices};
            for age_idx = 1:num_cases
                indices = age_categories{age_idx};
                if ~isempty(indices)
                    part_pos_x_age = part_pos_x(indices);
                    part_pos_y_age = part_pos_y(indices);
                    particle_density_age = histcounts2(part_pos_x_age, part_pos_y_age, x_edges, y_edges);
                    part_dens_age = (particle_density_age * count_per_particle) / cell_area;
                    part_conc_2D_age = part_dens_age ./ (Bl_depth(t) * rho_mg);
                    coarse_part_conc_2D_age = zeros(x_blocks, y_blocks);
                    for i = 1:x_blocks
                        for j = 1:y_blocks
                            x_start = (i-1)*block_size + 1;
                            x_end = i*block_size;
                            y_start = (j-1)*block_size + 1;
                            y_end = j*block_size;
                            block = part_conc_2D_age(x_start:x_end, y_start:y_end);
                            coarse_part_conc_2D_age(i, j) = mean(block(:));
                        end
                    end
                    part_conc_2D_t_age = coarse_part_conc_2D_age;
                    %part_conc_2D_t_age = part_conc_2D_t; 

                    % Twomey calculation for this age category
                    for i = 1:200
                        for j = 1:200
                            if part_conc_2D_t_age(i, j) ~= 0.0
                                delta_ac(i, j, age_idx) = (coarse_REFLECT(i, j)*(1-coarse_REFLECT(i, j))*...
                                    ((((part_conc_2D_t_age(i, j)+Nd_background)/Nd_background)^(1/3))-1)) / ...
                                    (1 + coarse_REFLECT(i, j)*((((part_conc_2D_t_age(i, j)+Nd_background)/Nd_background)^(1/3))-1));
                            else
                                delta_ac(i, j, age_idx) = 0.0;
                            end
                        end
                    end
                else
                    delta_ac(:,:,age_idx) = 0.0;
                end
            end
            delta_ac_all(:, :, t, :) = delta_ac;

            % Compute radiative forcing for each age category
            rad_forcing = zeros(200, 200, num_cases);
            for age_idx = 1:num_cases
                rad_forcing(:,:,age_idx) = coarse_SOLIN .* delta_ac(:,:,age_idx) .* phi;
            end
            rad_forcing_all(:, :, t, :) = -rad_forcing;

            % Domain-average Twomey forcing for each age category
            for age_idx = 1:num_cases
                domain_twomey_all(t, age_idx, les_idx) = mean(rad_forcing(:,:,age_idx), 'all', 'omitnan');
                rad_forcing_full(:,:,t,age_idx, les_idx) = rad_forcing(:,:,age_idx);
            end
        end
    end
...
%save the particle forcing per age group
for les_idx =1:num_les_cases
part_age_young(les_idx) = mean(domain_twomey_all(end-71:end,1,les_idx))/(mean(domain_twomey_all(end-71:end,1,les_idx))+mean(domain_twomey_all(end-71:end,2,les_idx))+mean(domain_twomey_all(end-71:end,3,les_idx)));
part_age_mid(les_idx) = mean(domain_twomey_all(end-71:end,2,les_idx))/(mean(domain_twomey_all(end-71:end,1,les_idx))+mean(domain_twomey_all(end-71:end,2,les_idx))+mean(domain_twomey_all(end-71:end,3,les_idx)));
part_age_old(les_idx) = mean(domain_twomey_all(end-71:end,3,les_idx))/(mean(domain_twomey_all(end-71:end,1,les_idx))+mean(domain_twomey_all(end-71:end,2,les_idx))+mean(domain_twomey_all(end-71:end,3,les_idx)));
end

young_frac = mean(part_age_young);
mid_frac = mean(part_age_mid);
old_frac = mean(part_age_old);